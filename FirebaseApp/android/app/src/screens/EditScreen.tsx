...
const EditScreen = ({ route, navigation }: any) => {
    ...
    // Xử lý lưu hoặc cập nhật ghi chú vào Firestore 
    const handleSave = async () => {
        try {
            if (!note.id) {
                // Tạo ghi chú mới với màu sắc được truyền từ HomeScreen
                ...
                // Cập nhật nội dung cho ghi chú hiện có
    ...
    // Xóa ghi chú ngay tại màn hình Edit [cite: 59]
    const handleDelete = () => {
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: note.color || '#FFF' }]}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
                ...
                {/* Toolbar mô phỏng giao diện Screen 2 [cite: 39, 40] */}
